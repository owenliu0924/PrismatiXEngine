...
inline bool FadeIn(float& alpha, float step, float maxAlpha = 255.0f) { return MoveTowards(alpha, maxAlpha, step); }

inline bool FadeOut(float& alpha, float step) { return MoveTowards(alpha, 0.0f, step); }
...
struct Transition {
    ...
    bool IsActive() const { return isEntering || isLeaving; }
    ...
    void Draw(SDL_Renderer* renderer, int width, int height) const { DrawOverlay(renderer, type, static_cast<Uint8>(alpha), width, height); }
};
