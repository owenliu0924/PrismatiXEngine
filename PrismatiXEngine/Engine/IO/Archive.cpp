...
constexpr char kMagic[4] = { 'P', 'D', 'X', '2' };
...
constexpr char kIndexSalt[] = "__pdx2_index__";
...
struct Cursor {
    ...
    [[nodiscard]] bool Ok(std::size_t n) const { return p + n <= end; }
    std::uint16_t U16() {
...
bool Archive::Contains(std::string_view path) const {
    return m_index.find(crypto::HashPath(NormalizePath(path))) != m_index.end();
}
...
std::optional<Bytes> Archive::Read(std::string_view path) const {
    ...
    auto it = m_index.find(crypto::HashPath(NormalizePath(path)));
    if (it == m_index.end()) {
        return std::nullopt;
    }
...
bool ArchiveWriter::Write(const std::string& outPath) const {
    ...
    for (const Pending& p : m_pending) {
        ...
        PutU64(index, crypto::HashPath(p.name));
