...
Stage::Stage(gfx::Renderer2D& renderer, gfx::AssetCache& assets)
...
void Stage::Render() {
    if (!m_prevBgPath.empty() && m_bgFade < 1.0f) {
        m_renderer.DrawImageAuto(m_prevBgPath, gfx::DisplayMode::Fill, 255);
    }
    if (!m_bgPath.empty()) {
        const auto alpha = static_cast<std::uint8_t>(std::clamp(m_bgFade, 0.0f, 1.0f) * 255.0f);
        m_renderer.DrawImageAuto(m_bgPath, gfx::DisplayMode::Fill, alpha);
    }
    ...
    for (const Actor* a : ordered) {
        ...
        int tw = 0, th = 0;
        gfx::AssetCache::TextureSize(tex, tw, th);
        if (tw <= 0 || th <= 0) {
            continue;
        }
