...
SDL_FColor ToFColor(Color c, float alphaScale = 1.0f) {
    return SDL_FColor{ c.r / 255.0f, c.g / 255.0f, c.b / 255.0f, (c.a / 255.0f) * alphaScale };
}

void AppendArc(std::vector<SDL_Vertex>& out, float cx, float cy, float radius, float a0, float a1,
               SDL_FColor color, int segments) {
...
std::string MakeKey(const std::string& text, const std::string& font, int size, Color c,
                    int outline, int wrap) {
    return text + "\x01" + font + "\x01" + std::to_string(size) + "\x01" +
           std::to_string(c.r) + "," + std::to_string(c.g) + "," + std::to_string(c.b) + "," +
           std::to_string(c.a) + "\x01" + std::to_string(outline) + "\x01" + std::to_string(wrap);
}
}
...
void Renderer2D::DrawImage(const std::string& path, const Rect& dst, std::uint8_t alpha) {
    Blit(m_assets.Texture(path), dst, alpha);
}

Rect Renderer2D::DrawImageAuto(const std::string& path, DisplayMode mode, std::uint8_t alpha,
                               int offsetX, int offsetY, float scale, Shadow shadow) {
    ...
    switch (mode) {
        case DisplayMode::TopLeft: x = 0, y = 0; break;
        case DisplayMode::TopRight: x = rw - w, y = 0; break;
        case DisplayMode::BottomLeft: x = 0, y = rh - h; break;
        case DisplayMode::BottomRight: x = rw - w, y = rh - h; break;
        case DisplayMode::Top: x = (rw - w) * 0.5f, y = 0; break;
        case DisplayMode::Bottom: x = (rw - w) * 0.5f, y = rh - h; break;
        case DisplayMode::Left: x = 0, y = (rh - h) * 0.5f; break;
        case DisplayMode::Right: x = rw - w, y = (rh - h) * 0.5f; break;
        case DisplayMode::Center: x = (rw - w) * 0.5f, y = (rh - h) * 0.5f; break;
...
const Renderer2D::CachedText* Renderer2D::AcquireText(const std::string& text,
                                                      const std::string& fontPath, int size,
                                                      Color color, int outline, int wrap) {
    const std::string key = MakeKey(text, fontPath, size, color, outline, wrap);
    if (auto it = m_textCache.find(key); it != m_textCache.end()) {
        return &it->second;
    }

    TTF_Font* font = m_assets.Font(fontPath, size, outline);
    if (!font || text.empty()) {
        return nullptr;
    }
    ...
    SDL_Surface* surface =
        wrap > 0 ? TTF_RenderText_Blended_Wrapped(font, text.c_str(), text.size(), col, wrap)
                 : TTF_RenderText_Blended(font, text.c_str(), text.size(), col);
    if (!surface) {
        return nullptr;
    }
...
void Renderer2D::DrawText(const std::string& text, float x, float y, const std::string& fontPath,
                          int size, Color color, std::uint8_t alpha, int wrap) {
...
void Renderer2D::DrawTextOutline(const std::string& text, float x, float y,
                                 const std::string& fontPath, int size, Color textColor,
                                 Color outlineColor, int outlineSize, std::uint8_t alpha,
                                 bool shadow, int wrap) {
    if (shadow) {
        ...
        if (s) {
            ...
            Blit(s->texture,
                 Rect{ x + 3, y + 3, static_cast<float>(s->w), static_cast<float>(s->h) }, sa);
        }
    }
    ...
    if (outlineSize > 0) {
        ...
        if (o) {
            Blit(o->texture, Rect{ x, y, static_cast<float>(o->w), static_cast<float>(o->h) },
                 alpha);
        }
    }
    ...
    if (f) {
        ...
        Blit(f->texture,
             Rect{ x + ox, y + ox, static_cast<float>(f->w), static_cast<float>(f->h) }, alpha);
    }
}

Vec2 Renderer2D::MeasureText(const std::string& text, const std::string& fontPath, int size,
                             int wrap) {
