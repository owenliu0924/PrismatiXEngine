...
RenderSystem::RenderSystem(SDL_Renderer* ren, AssetManager& assetMgr) : renderer(ren), assetManager(assetMgr) {}
...
SDL_Rect RenderSystem::DrawTextureAuto(SDL_Texture* tex, DisplayMode mode, Uint8 alpha, int offsetX, int offsetY, float scale, ShadowConfig shadow) {
    ...
    switch (mode) {
        case DisplayMode::TopLeft: destRect.x = 0; destRect.y = 0; break;
        case DisplayMode::TopRight: destRect.x = renderW - scaledW; destRect.y = 0; break;
        case DisplayMode::BottomLeft: destRect.x = 0; destRect.y = renderH - scaledH; break;
        case DisplayMode::BottomRight: destRect.x = renderW - scaledW; destRect.y = renderH - scaledH; break;
        case DisplayMode::Top: destRect.x = (renderW - scaledW) / 2; destRect.y = 0; break;
        case DisplayMode::Bottom: destRect.x = (renderW - scaledW) / 2; destRect.y = renderH - scaledH; break;
        case DisplayMode::Left: destRect.x = 0; destRect.y = (renderH - scaledH) / 2; break;
        case DisplayMode::Right: destRect.x = renderW - scaledW; destRect.y = (renderH - scaledH) / 2; break;
        case DisplayMode::Center: destRect.x = (renderW - scaledW) / 2; destRect.y = (renderH - scaledH) / 2; break;
...
void RenderSystem::DrawTextWithOutline(TTF_Font* font, const std::string& text, SDL_Color textColor, SDL_Color outlineColor, int outlineSize, int x, int y, Uint32 wrapLength, Uint8 alpha, bool shadow) {
    ...
    TTF_Font* outlineFont = assetManager.GetOutlineFont(font, outlineSize);
