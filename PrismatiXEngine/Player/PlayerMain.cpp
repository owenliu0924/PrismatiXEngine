...
int RunSelfTest() {
    ...
    {
        px::progress::GlobalProfile p;
        px::script::LuaServices services;
        services.profile = &p;
        px::script::LuaHost lua(services);
...
int main(int argc, char* argv[]) {
    ...
    px::script::LuaServices luaServices;
    ...
    px::script::LuaHost lua(luaServices);
    ...
    while (runtime.BeginFrame()) {
        ...
        if (appState == AppState::Title) {
            ...
            if (saveRequested) {
                ...
                snap.thumbnailPng = px::gfx::CaptureThumbnailPng(runtime.Renderer().Handle(), 256, 144);
                saves.Save(0, snap);
