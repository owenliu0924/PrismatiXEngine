...
void TestDesignerRewriteContracts(){
    ...
    Check(properties.RuntimeSupports("background.color","Button")&&properties.RuntimeSupports("padding","Button")&&!properties.RuntimeSupports("opacity","Button"),"Style Inspector support must match the explicit renderer-backed property allowlist");
    bool styleSupportExact=true;const auto mapped=px::ui::RuntimeMappedStyleProperties();for(const auto* descriptor:properties.Descriptors()){const bool hasMapper=std::find(mapped.begin(),mapped.end(),descriptor->id)!=mapped.end();styleSupportExact&=descriptor->runtimeSupported==hasMapper;}Check(styleSupportExact,"every Style Inspector runtimeSupported flag must have an exact runtime mapper and vice versa");
