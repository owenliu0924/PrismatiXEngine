...
id = "ending-rin"
title = "Rin Ending"
image = res("33333333-3333-4333-8333-333333333333", "Assets/rin.png")
thumbnail = res("33333333-3333-4333-8333-333333333333", "Assets/rin.png")
)";
}

std::string PropertyText(const px::resource::TypedDocument& document, const std::string& name) {
    const auto found = document.properties.find(name);
    if (found == document.properties.end()) return {};
...
int main() {
    ...
    Write(root / "project.pxproject",
      ...
      "entryChapterId":"88888888-8888-4888-8888-888888888888",
      "chapters":[{
        "id":"88888888-8888-4888-8888-888888888888",
        "title":"Entry Chapter",
        "entrySceneId":"99999999-9999-4999-8999-999999999999",
        "scenes":[{
          "id":"99999999-9999-4999-8999-999999999999",
          "title":"Entry Scene",
          "source":"Story/Entry.pxstory",
          "performance":"Story/Entry.pxperformance"
        }]
      }],
      "assets":[{
    ...
    suite.Run("StrictRequest_ParsesLockedWireContract", [&] {
        ...
        json << R"({"format":"PrismatiXPackageRequest","schemaRevision":1,)"
             ...
             << R"("contentVersion":"v1","encryption":false,)"
             << R"("compression":"balanced","inputs":[)"
             << R"({"uri":"Content/Runtime/start.pxir","fingerprint":")" << request.inputs[4].fingerprint << R"(","size":)" << request.inputs[4].size << R"(}],)"
    ...
    suite.Run("Package_CreatesPlayerCompatibleAtomicDistribution", [&] {
        ...
        suite.Expect(std::filesystem::is_regular_file(output / "game.pxpackage"), "typed game.pxpackage is promoted");
        ...
        const auto manifest = px::resource::ParseTypedDocument(Read(output / "game.pxpackage"), (output / "game.pxpackage").generic_string());
        suite.Require(static_cast<bool>(manifest), "Player parses the generated typed package");
        suite.Expect(
            manifest.Value().type == "GamePackage" && PropertyText(manifest.Value(), "startScript") == request.startScript && PropertyText(manifest.Value(), "startRoute") == "title", "typed package retains the compiled entrypoint and start route"
        );
        const auto routes = manifest.Value().properties.find("routes");
        const auto* routeArray = routes == manifest.Value().properties.end() ? nullptr : routes->second.AsArray();
        ...
        const auto packagedCatalog = archive.Read("Content/Game.pxres");
        suite.Require(packagedCatalog.has_value(),
                      "Player can read the packaged runtime GameCatalog");
        const auto catalog = px::sdk::LoadGameCatalogResources(
        ...
        suite.Expect(
            ...
            "Packager and Player share residual GameCatalog semantics");
...
default = "seven"
        ...
        suite.Expect(
            result.exitCode == px::sdk::PackageExitCode::Failed &&
                HasDiagnostic(result, "PXCAT1012"),
            "Packager executes the shared typed GameCatalog contract before staging");
    });
    ...
    suite.Run("GalleryPathsAndUnknownUuids_AreRejectedBeforePackaging", [&] {
        ...
        Write(catalogPath, R"(
@pxresource 4 00000000-0000-4000-8000-000000000001 GameCatalog
[node "00000000-0000-4000-8000-000000000012", "", "LegacyGallery", "GalleryItem"]
id = "legacy"
image = "Assets/rin.png"
        ...
        suite.Expect(
            legacyResult.exitCode == px::sdk::PackageExitCode::Failed &&
                HasDiagnostic(legacyResult, "PXCAT1019"),
            "characterResources projects cannot package path-authoritative Gallery items");

        Write(catalogPath, R"(
@pxresource 4 00000000-0000-4000-8000-000000000001 GameCatalog
[node "00000000-0000-4000-8000-000000000012", "", "UnknownGallery", "GalleryItem"]
id = "unknown"
image = res("99999999-9999-4999-8999-999999999999", "Assets/rin.png")
)" );
    ...
    suite.Run("MalformedStudioUiRoute_IsRejected", [&] {
        ...
        malformed.routes.front().scene = "Content/UI/Malformed.pxui";
        const auto routeInput = std::find_if(
            malformed.inputs.begin(), malformed.inputs.end(),
            [](const auto& input) {
                return input.uri == "Content/UI/Title.pxui";
            });
        suite.Require(routeInput != malformed.inputs.end(),
                      "fixture contains the original route input");
        *routeInput = Input(root, "Content/UI/Malformed.pxui");
