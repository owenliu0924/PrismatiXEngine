...
void BehaviorGraphEditor::FocusNode(const Uuid& node){m_focusNode=node;m_frameSelection=true;}
void BehaviorGraphEditor::ClearSelection(){
    m_selectedNode={};m_selectedGroup={};m_focusNode={};
    if(m_context){ed::SetCurrentEditor(m_context);ed::ClearSelection();ed::SetCurrentEditor(nullptr);}
}

bool BehaviorGraphEditor::Render(UISceneDocument& document,const Uuid& selectedControl){
    ...
    if(ImGui::Button("＋ 節點")){m_pendingCreatePin=0;const ImVec2 mouse=ImGui::GetMousePos();m_pendingCreateX=mouse.x;m_pendingCreateY=mouse.y;ImGui::OpenPopup("新增 Behavior 節點");}ImGui::SameLine();if(ImGui::Button("＋ 群組／註解")){const Uuid id=Uuid::Random();const float offset=40.0f*static_cast<float>(graph.groups.size()%8);graph.groups.push_back({id,"註解",{60+offset,60+offset,360,220}});m_selectedGroup=id;m_selectedNode={};dirty=true;}ImGui::SameLine();if(ImGui::Button("框選內容")){m_frameSelection=true;}ImGui::SameLine();ImGui::TextDisabled("右鍵或從 pin 拖到空白處開啟 Palette；Ctrl+C / Ctrl+V 複製貼上");if(!m_debugState.fibers.empty()){ImGui::SameLine();ImGui::TextColored({1.0f,.7f,.25f,1.0f},"● %zu fibers",m_debugState.fibers.size());}
    ...
    ed::SetCurrentEditor(nullptr);return dirty&&Commit(document,before,graph,"Edit Behavior Graph");
}

bool BehaviorGraphEditor::RenderInspector(UISceneDocument& document,const Uuid& selectedControl){auto parsed=ReadGraph(document);if(!parsed){ImGui::TextDisabled("Behavior Graph 無法解析。");return false;}auto graph=parsed.TakeValue();const Variant before=GraphBefore(document);if(m_selectedNode.Empty()){const auto group=std::find_if(graph.groups.begin(),graph.groups.end(),[&](const ui::BehaviorGroup& item){return item.id==m_selectedGroup;});if(group==graph.groups.end()){ImGui::TextDisabled("選取 Behavior 節點或群組以編輯屬性。");return false;}std::string title=group->title;if(ImGui::InputText("群組／註解標題",&title)){group->title=std::move(title);return Commit(document,before,graph,"Edit Behavior Group");}ImGui::Text("UUID  %s",group->id.ToString().c_str());return false;}auto found=std::find_if(graph.nodes.begin(),graph.nodes.end(),[&](const ui::BehaviorNode& node){return node.id==m_selectedNode;});if(found==graph.nodes.end())return false;auto& node=*found;bool changed=false;ImGui::TextDisabled("%s",ui::BehaviorNodeKindName(node.kind));ImGui::Text("UUID  %s",node.id.ToString().c_str());
    ...
    return changed&&Commit(document,before,graph,"Edit Behavior Node");}
